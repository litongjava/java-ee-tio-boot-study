...
public class ShrugCommand implements Command {
  ...
  public Publisher<?> execute(SendMessageEvent event) {
    return Mono.justOrEmpty(event.getChat()).switchIfEmpty(event.getMessage().getChat())
        //
        .flatMap(c -> c.sendMessage(
            //
            SendMessageSpec.builder().message("¯\\_(ツ)_/¯")
                //
                .replyTo(ReplyToMessageSpec.of(event.getMessage())).build()));
  }
}